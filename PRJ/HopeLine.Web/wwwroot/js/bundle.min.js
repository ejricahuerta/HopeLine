"use strict";var currentuser="",connection,isconnected=!1,room=$("#pin").val();connection=(new signalR.HubConnectionBuilder).withUrl("http://localhost:5000/chatHub").build();connection.start().catch(function(n){return console.error(n.toString())});connection.on("Receivemessage",function(n,t){var i=currentuser==n?"bg-secondary":"bg-info";$("#chatbox").append('<div id="message"><span class= "badge">'+n+'<\/span> <div class="'+i+' col-11 mb-1 rounded"> <p class="p-2">'+t+"<\/p><\/div><\/div>")});$("#userInput").click(function(){isconnected||(connection.invoke("AddUserToRoom",room).catch(function(n){return console.error(n.toString())}),isconnected=!0)});$("#sendButton").click(function(){currentuser=$("#userInput").val();var n=$("#messageInput").val();$("#messageInput").val(" ");connection.invoke("SendMessage",currentuser,n,room).catch(function(n){return console.error(n.toString())});event.preventDefault()});
/** 
