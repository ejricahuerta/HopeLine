...
namespace HopeLine.API.Hubs {
    ...
    public class ChatHub : Hub {
        ...
        public bool Reconnect (string userId, bool isGuest) {
            System.Console.WriteLine ("Attempting to Reconnect...");
            var room = _messageService.GetRoomForUser (userId, isGuest);
            if (room != null) {
                Groups.AddToGroupAsync (Context.ConnectionId, room);
                Clients.Group (room).SendAsync ("ReceiveMessage", "system", "Reconnecting..");
                var allMessages = _messageService.GetAllMessages (room);
                System.Console.WriteLine (" Count: " + allMessages.Count ());
                if (allMessages != null) {
                    foreach (var m in allMessages) {
                        Clients.Caller.SendAsync ("Load", m.UserName, m.Text);
                    }
                }
                return true;
            }
        ...
        public async Task UserRequestToTalk (string user) {
            ...
            if (availablementors == null) {
                ...
                await Clients.Clients (availablementors)
                    .SendAsync ("NotifyMentor", user, Context.ConnectionId);
        ...
        public async Task AddMentor (string mentorId) {
            ...
            try {
                var mentor = _messageService.ListAvailableMentor ().SingleOrDefault (m => m.MentorId == mentorId);
                if (mentor != null) {
                    mentor.ConnectionId = Context.ConnectionId;
                }
        ...
        public async Task MentorAcceptRequest (string mentorId, string userId, string userConnectionId) {
            try {
                //Generate room
                ...
                await Clients.Group (room).SendAsync ("ReceiveMessage", mentorId, "Hello!");
            } catch (System.Exception ex) {
